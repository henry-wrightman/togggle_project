...
export class AppService {
  ...
  async getTokenData(): Promise<TokenDataResponse | Error> {
    const result = (await this.tokenService.getTokenData()) as TokenDataResponse;
    if (!result) {
  ...
  async addGuess(playerId: number, guess: number, initialPrice: number): Promise<void | Error> {
    ...
    const callback = async (winnings, finalPrice) => {
      ...
      await this.databaseService.updateGuess(playerId, newGuess.id, initialPrice, finalPrice, winnings === 1);
    };
