...
it('initial GuessTimer state', async () => {
  ...
  await waitFor(
    () => expect(screen.findByText(/until guess resolves/)).rejects, // not found
    { timeout: 3000 }
  );
});
...
it('active GuessTimer state', async () => {
  ...
  await waitFor(() => expect(screen.findByText(/until guess resolves/)).resolves.not.toBeNull(), { timeout: 3000 });
});
